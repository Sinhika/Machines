 ...
 * @author Cadiboo
 */
public abstract class OnyxFurnaceTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider 
{
	...
	{
		super(ModTileEntityTypes.onyx_furnace.get());
	}
	...
	public void tick() {
		...
		if (lastBurning != hasFuel) { // We use hasFuel because the current fuel may be all burnt out but we have more that will be used next tick
			...
			final BlockState newState = this.getBlockState()
					.with(OnyxFurnaceBlock.BURNING, hasFuel);
